...
The wrapper-lifetime check also verifies lookup through the actual shared dictionary used by the older baseline Harmony assembly. It does not claim that all of that older engine's public APIs work side by side: initializing its separate MonoMod implementation encounters an existing proxy-type conflict in this fixture. Older .NET Framework and Mono runtime paths were not executed in the initial local pass. Existing broader runtime evidence and limits remain in [docs/infix/README.md](../../docs/infix/README.md).

The [first platform CI run](https://github.com/pardeike/Harmony/actions/runs/34197810834) exposed an overbroad emitted-callback proxy path on Framework and Mono. The follow-up limits these proxies to runtimes with `AssemblyLoadContext`, preserving the existing rejection of competing assembly identities elsewhere. The unchanged loading tests and emitted-callback tests pass locally on Mono 6.12.0.206 x64 with the net472 build, 32 cases, and .NET 9.0.19 x64, 33 cases. The separate [Infix compatibility run](https://github.com/pardeike/Harmony/actions/runs/34197810717) passed all 16 jobs on the initial commit.
