...
The existing source-baseline CI jobs now run the extension and completion checks against both pinned prior formats. For completion commit `1ffe968`, [compatibility CI](https://github.com/pardeike/Harmony/actions/runs/34137951649) passed all 16 jobs, including the required Windows Framework and completion coexistence cases. Documentation build and deployment also passed. These results do not establish Unity support or turn classified Mono loading limitations into successful coexistence.

The first [platform run](https://github.com/pardeike/Harmony/actions/runs/34137951945) produced all 128 expected completed framework reports but failed 105 test executions for three reasons. Windows Framework hit an unrelated MonoMod limitation in the constrained fixture's generic outer method; net7 JSON tests used the test assembly's serializer instead of Harmony's internalized serializer; and older Release runtimes exposed an uninitialized inner result after a caught fault. The follow-up corrections retain the constrained instruction in a nongeneric outer fixture, test real production serialization with precise malformed-state assertions, and preserve explicit helper initialization with `DynamicMethod.InitLocals = false`. No tests were skipped or CI gates relaxed. The [testing strategy](TESTING-STRATEGY.md) records the regression requirements; passing results from the first run do not validate these corrections.

The corrected candidate passes fresh complete x64 Release suites on .NET 3.1.32 (866), 5.0.17 (893), 7.0.20 (893), 8.0.30 (905) and 9.0.19 (906), plus .NET 9 Debug (906) and Mono 6.12/net452 Debug (876). Every suite has zero failures; existing exclusions are unchanged. Debug and Release builds cover all configured frameworks. A separate net5 Release finalizer repeat passes 77 cases, and freshly rebuilt distinct engines pass all three net9/JSON completion children against the version-2 baseline, including same-name callback identity during cold rebuilding. The corrections also received a separate local-initialization and correctness review. Remote CI must validate this candidate independently.
